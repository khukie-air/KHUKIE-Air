...
def convert_to_s3_trash_key(UUID, object_suffix):
    return get_trashbox_path(UUID)+object_suffix


def convert_to_s3_key(UUID, logical_path):
    return get_root_path(UUID)+logical_path


def convert_to_logical_path(UUID, s3_key):
    return s3_key.replace(get_root_path(UUID), '')


def get_root_path(UUID):
    return UUID+'/root/'


def get_trashbox_path(UUID):
    return UUID+'/trashbox/'


def get_file(pk, UUID):
    try:
        file = File.objects.get(file_id=pk)
        if UUID not in file.path:
            raise PermissionDenied
        return file
...
def get_folder(pk, UUID):
    try:
        folder = Folder.objects.get(folder_id=pk)
        if UUID not in folder.path:
            raise PermissionDenied
        return folder
    except ObjectDoesNotExist:
        return None
...
def get_trash(pk, UUID):
    try:
        trash = Trash.objects.get(trash_id=pk)
        if UUID not in trash.original_path:
            raise PermissionDenied
        return trash
    except ObjectDoesNotExist:
        return None
...
class FileRetrieveCopyDelete(APIView):
    ...
    def post(self,request,pk,format=None):
        ...
        dest_folder = get_folder(dest_folder_id, UUID)
        ...
        new_file.save()
        new_file.path = convert_to_logical_path(UUID, new_file.path)
    ...
    def delete(self, request, pk, format=None):
        ...
        file.delete()
        trash.path = convert_to_logical_path(UUID, trash.path)
...
class FileUploadLinkCreate(APIView):
    ...
    def post(self,request,format=None):
        ...
        if 'size' in attributes:
            ...
            return Response(status=status.HTTP_400_BAD_REQUEST)
        parent_folder = get_folder(loc_folder_id,UUID)
...
class FileRename(APIView):
    def put(self,request,pk,format=None):
        ...
        s3_bucket_sdk.rename_or_move_file(s3,old_key=old_key, new_key=new_key)
        file.save()
        file.path = convert_to_logical_path(UUID,file.path)
...
class FolderCreate(APIView):
    def post(self,request,format=None):
        ...
        if folder_name is None:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        parent_folder = get_folder(parent_folder_id,UUID)
        if parent_folder is None:
            return Response(status=status.HTTP_404_NOT_FOUND)
        ...
        folder.path = convert_to_logical_path(UUID, folder_path)
...
class FolderRetrieveCopyDelete(APIView):
    def get(self,request,pk,format=None):
        ...
        if folder is not None:
            fields = request.data['fields'].split(',') if 'fields' in request.data else None
            folder.path = convert_to_logical_path(UUID, folder.path)
    ...
    def post(self,request,pk,format=None):
        ...
        s3_bucket_sdk.copy_folder(s3, old_key=old_key, destination_prefix=dest_folder.path, folder_name=folder.folder_name)
    ...
    def delete(self,request,pk,format=None):
        ...
        new_key = convert_to_s3_key(UUID, old_key)
        s3_bucket_sdk.rename_or_move_folder(s3,old_key=old_key, old_folder_name=old_folder_name, new_key=new_key)

        folder.delete()
        folder_trash.original_path=convert_to_logical_path(UUID,folder_trash.original_path)
...
class FolderItemList(APIView):
    def get(self,request,pk,format=None):
        ...
        for i in range(all_items):
            all_items[i].path = convert_to_logical_path(UUID, all_items[i].path)
...
def expire_trash(UUID):
    trashes = Trash.objects.filter(expire_time__lte=datetime.datetime.now(), cascade_trash=None, original_path__startswith=UUID)
...
class TrashList(APIView):
    ...
    def get(self,request,format=None):
        ...
        offset = request.data['offset'] if 'offset' in request.data else 0
        if 'sort' == 'name':
            trashes = Trash.objects.filter(cascade_trash=None, original_path__startswith=UUID).order_by('obj_name')
        else:
            trashes = Trash.objects.filter(cascade_trash=None, original_path__startswith=UUID).order_by('trashed_at')
        trashes = trashes[offset:offset + limit]
        for i in range(len(trashes)):
            trashes[i].original_path = convert_to_logical_path(UUID,trashes[i].original_path)
...
class TrashControl(APIView):
    def put(self,request,pk,format=None):
        ...
        if trash.type =="file":
            ...
            s3_bucket_sdk.rename_or_move_file(s3, old_key=get_trashbox_path(UUID)+trash.obj_name,new_key=recovered_file.path)
            middle_folder_create(recovered_file,s3_client)
            recovered_file.save()
            recoverd_file.path=convert_to_logical_path(UUID,recovered_file)
            ...
            for sub_trash in Trash.objects.filter(cascade_trash=trash, type='folder', original_path__startswith=UUID):
                try:
            ...
            for sub_trash in Trash.objects.filter(cascade_trash=trash, type='file', original_path__startswith=UUID):
                ...
                s3_bucket_sdk.rename_or_move_file(s3, old_key=get_trashbox_path(UUID)+sub_trash.obj_name,new_key=recovered_file.path)
            recovered_parent_folder.path = convert_to_logical_path(UUID,recovered_parent_folder.path)
    ...
    def delete(self,request,pk,format=None):
        ...
        if trash.cascade_trash is not None:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        s3_bucket_sdk.remove_file(s3,get_trashbox_path(UUID)+trash.obj_name)
        trash.delete()
        return Response(status=200)
